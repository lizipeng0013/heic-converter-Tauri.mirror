...
describe('MainView.vue', () => {
  ...
  it('renders dialog components', () => {
    ...
    expect(titleBar.exists()).toBe(true)
    expect(titleBar.props('appName')).toBe('HEIC 图片格式转换器')
    expect(titleBar.props('showTrayButton')).toBe(true)
    expect(titleBar.props('height')).toBe('medium')
  })
  ...
  it('renders dialog components', () => {
    ...
    expect(wrapper.findComponent({ name: 'ConvertingCloseConfirmDialog' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'PendingFilesCloseConfirmDialog' }).exists()).toBe(true)
  })
})
