...
const handleMouseUp = () => {
  mouseDownPosition.value = null; isTitleDragging.value = false; 
}
...
<template>
  <header class="h-14 px-4 flex items-center justify-between shrink-0 select-none relative z-50" :class="isDark ? 'bg-slate-900 border-b border-slate-700' : 'bg-white border-b border-slate-200'">
    <!-- 标题栏可拖拽区域 -->
    <div
    ...
    >
      <!-- 左侧Logo -->
      <div class="flex items-center gap-2 font-semibold text-lg tracking-tight text-foreground">
      ...
      <div class="flex-1 h-full">
        <ContextMenu>
          ...
          <ContextMenuContent class="w-48">
            <ContextMenuItem @click="maxWindow"><Maximize2 class="mr-2 h-4 w-4" /> 最大化/还原</ContextMenuItem>
            <ContextMenuItem @click="minWindow"><Minus class="mr-2 h-4 w-4" /> 最小化</ContextMenuItem>
            <ContextMenuItem @click="toggleTop"><Pin class="mr-2 h-4 w-4" /> 置顶窗口</ContextMenuItem>
            <ContextMenuSeparator />
            <ContextMenuItem class="text-destructive" @click="closeWindow"><X class="mr-2 h-4 w-4" /> 关闭</ContextMenuItem>
          </ContextMenuContent>
        </ContextMenu>
      </div>
    </div>
