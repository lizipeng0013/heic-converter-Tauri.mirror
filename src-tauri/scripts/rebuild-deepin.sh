...
# 目标目录（在工作目录中的完整路径）
TARGET_DIR="${WORK_DIR}/deepin${INSTALL_PREFIX}"
...
# 1. 查找 bundle 目录
# ============================================
BUNDLE_DIR="$PROJECT_ROOT/target/release/bundle/deb/${APPNAME}_${VERSION}_${ARCH}"
...
if [ -f "usr/bin/${APPNAME}" ]; then
    ...
    mv "usr/bin/${APPNAME}" "${TARGET_DIR}/files/"
    echo "  → 可执行文件: ${INSTALL_PREFIX}/files/${APPNAME}"
elif [ -f "usr/bin/heic-converter" ]; then
    # 备用：如果旧名称存在
    mkdir -p "${TARGET_DIR}/files"
    mv "usr/bin/heic-converter" "${TARGET_DIR}/files/${APPNAME}"
...
if [ -d "usr/share/icons" ]; then
    find "usr/share/icons" -type f \( -name "*.png" -o -name "*.svg" \) | while read icon; do
        ...
        target_dir="${TARGET_DIR}/entries/icons/$(dirname "$rel_path")"
        target_dir=$(echo "$target_dir" | sed 's/@2//')
        mkdir -p "$target_dir"
        mv "$icon" "$target_dir/${APPID}.png"
...
if [ -f "usr/share/applications/${APPNAME}.desktop" ]; then
    mkdir -p "${TARGET_DIR}/entries/applications"
    mv "usr/share/applications/${APPNAME}.desktop" "${TARGET_DIR}/entries/applications/"
    ...
    # 备用：如果旧名称存在
    mkdir -p "${TARGET_DIR}/entries/applications"
    mv "usr/share/applications/heic-converter.desktop" "${TARGET_DIR}/entries/applications/${APPID}.desktop"
...
if [ -d "${TARGET_DIR}/entries/applications" ]; then
    for desktop_file in "${TARGET_DIR}/entries/applications/"*.desktop; do
...
if [ -f "$PROJECT_ROOT/scripts/info" ]; then
    mkdir -p "${TARGET_DIR}"
    cp -r "$PROJECT_ROOT/scripts/info" "${TARGET_DIR}/"
    echo "  → 已添加 info"
else
...
fakeroot dpkg-deb -b . "${PROJECT_ROOT}/target/release/bundle/deb/${APPNAME}_${VERSION}_deepin_${ARCH}.deb"
...
echo ""
echo "✅ 重建完成！"
echo "   输出: ${PROJECT_ROOT}/target/release/bundle/deb/${APPNAME}_${VERSION}_deepin_${ARCH}.deb"
echo ""
echo "验证命令："
echo "  dpkg-deb -c ${PROJECT_ROOT}/target/release/bundle/deb/${APPNAME}_${VERSION}_deepin_${ARCH}.deb | grep -A 20 'opt/apps'"
